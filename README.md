...
| `kb-mbin`                      | Background | Mbin federated content aggregator + microblogging (Kbin fork), hybrid dual content model (threads as Page + microblogs as Note), magazines as Group actors, separate boost/vote model (unlike Lemmy), Symfony Messenger async processing (15 inbox + 15 outbox message types), C4 collective governance, 106 entity classes, CLI admin commands, Mbin vs Lemmy comparison |
...
| `kb-nodebb`                    | Background | NodeBB modern forum software (Node.js), ActivityPub as core feature (v4+), categories as Group actors (FEP-1b12), topics as resolvable OrderedCollection contexts (FEP-7888), 12 implemented FEPs, content pruning, WebSocket real-time, Discourse comparison |
